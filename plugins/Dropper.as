...
bool blIsDropEnabled = g_EngineFuncs.CVarGetFloat("mp_dropweapons") == 1;
...
void MapInit()
{
    if( blIsDropEnabled )
    {
        float flRegisterhook = g_EngineFuncs.CVarGetFloat( "mp_survival_startdelay" );
        uint uiCount = 0;
        ...
        if( bEnabled )
		{
			g_Scheduler.SetTimeout( "RegisterHookDropper", flRegisterhook );
		}
        else
		{
			g_Hooks.RegisterHook( Hooks::Player::ClientSay, @ClientSayDropper2 );
		}
	}
}

void RegisterHookDropper()
{
	g_Hooks.RegisterHook( Hooks::Player::ClientSay, @ClientSayDropper );
	g_PlayerFuncs.ClientPrintAll( HUD_PRINTTALK, "Plugin Dropped Enabled.\n");
}

HookReturnCode ClientSayDropper2(SayParameters@ pParams)
{
	...
	if( str == "DROPAMMO" ){
		...
		g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "Plugin disabled on this map.\n");
	}
...
HookReturnCode ClientSayDropper(SayParameters@ pParams){
	...
	if( str == "DROPAMMO" )
	{
		...
		{
			g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "can't drop while dead.\n");
			return HOOK_CONTINUE;
		}
